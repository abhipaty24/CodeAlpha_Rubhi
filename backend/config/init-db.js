...
function initDatabase() {
  ...
  // Seed admin user
  const existing = db.prepare("SELECT id FROM users WHERE email = ?").get('mallariabhi@gmail.com');
  if (!existing) {
    const hash = bcrypt.hashSync('Abhi@2006', 12);
    db.prepare(`
      INSERT INTO users (full_name, username, email, password_hash, role, is_verified)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run('Admin', 'admin', 'mallariabhi@gmail.com', hash, 'admin', 1);
    console.log('  Admin account created: mallariabhi@gmail.com / Abhi@2006');
  }
